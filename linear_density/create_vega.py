...
def main():
    ...
    for i in range(N_sim):
        ...
        rpbins = np.linspace(0, 200, 201); assert want_fft # TESTING!!!! ok for FFT
        npibins = int(rpbins[-1]/(rpbins[1]-rpbins[0]))
        rpbinc = (rpbins[1:] + rpbins[:-1])*.5
        rp = np.tile(rpbinc, (rpbinc.shape[0], 1)).T
        rt = rp.T

        for los_dir in ["z", "y", "x"]:

            if want_fft:
                #save_dir = "/pscratch/sd/b/boryanah/AbacusLymanAlpha/correlations/Xi/z2.500/"
                save_dir = "data/"
                data = np.load(save_dir + f"Xi_{corr_type}_LyAxLyA_{sim_name}_linear_density_LOS{los_dir}_d4.0.npz")
            ...
            # now copy this on DESI data format
            ...
            h.writeto(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/{sim_name}/cf_lya_x_lya_abacus_linear_density_LOS{los_dir}{fft_str}.fits", overwrite=True)
    ...
    for r in range(N_sim):
        for los_dir in ["z", "y", "x"]:
            ...
            filename = f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/{sim_name}/cf_lya_x_lya_abacus_linear_density_LOS{los_dir}{fft_str}.fits"
    ...
    h.writeto(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/cf_lya_x_lya_abacus_mean_linear_density{fft_str}.fits", overwrite=True)

    # TESTING!!!!!
    r,xim,_,_ = compute_wedge(h["COR"].data["RP"],h["COR"].data["RT"],h["COR"].data["DA"],h["COR"].data["CO"],murange=[0.,0.5],rrange=[0,148])
    fitsio.write(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/cf_lya_x_lya_abacus_mean_linear_density_r{fft_str}.fits", r)
    fitsio.write(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/cf_lya_x_lya_abacus_mean_linear_density_0_5{fft_str}.fits", xim)

    r,xim,_,_ = compute_wedge(h["COR"].data["RP"],h["COR"].data["RT"],h["COR"].data["DA"],h["COR"].data["CO"],murange=[0.5,0.8],rrange=[0,148])
    fitsio.write(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/cf_lya_x_lya_abacus_mean_linear_density_5_8{fft_str}.fits", xim)

    r,xim,_,_ = compute_wedge(h["COR"].data["RP"],h["COR"].data["RT"],h["COR"].data["DA"],h["COR"].data["CO"],murange=[0.8,0.95],rrange=[0,148])
    fitsio.write(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/cf_lya_x_lya_abacus_mean_linear_density_8_95{fft_str}.fits", xim)

    r,xim,_,_ = compute_wedge(h["COR"].data["RP"],h["COR"].data["RT"],h["COR"].data["DA"],h["COR"].data["CO"],murange=[0.95,1.],rrange=[0,148])
    fitsio.write(f"/pscratch/sd/b/boryanah/abacus_tng_lyalpha/cf_lya_x_lya_abacus_mean_linear_density_95_1{fft_str}.fits", xim)
